...
RSpec.describe DaffyLib::KeyManagementService, type: :request do
  ...
  describe '#new' do
    it 'raises an exception when partition_guid is nil' do
      expect { described_class.new(nil, expires_in, cmk_key_id) }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end

    it 'raises an exception when partition_guid is blank' do
      expect { described_class.new('', expires_in, cmk_key_id) }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end

    it 'raises an exception when expires_in is nil' do
      expect { described_class.new(partition_guid, nil, cmk_key_id) }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end

    it 'raises an exception when expires_in is blank' do
      expect { described_class.new(partition_guid, '', cmk_key_id) }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end

    it 'raises an exception when cmk_key_id is nil' do
      expect { described_class.new(partition_guid, expires_in, nil) }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end

    it 'raises an exception when cmk_key_id is blank' do
      expect { described_class.new(partition_guid, expires_in, '') }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end
  end
  ...
  describe '#find_or_create_encryption_key' do
    ...
    it 'raises an exception when the call to PorkyLib::Symmetric fails' do
      ...
      expect { service.find_or_create_encryption_key(encryption_epoch) }.to raise_exception(DaffyLib::KeyManagementService::KeyGenerationException)
    end

    it 'raises an exception when the call to create an EncryptionKey fails (failure other than RecordInvalid)' do
      allow(DaffyLib::EncryptionKey).to receive(:create!).and_raise(ActiveRecord::ActiveRecordError)

      expect { service.find_or_create_encryption_key(encryption_epoch) }.to raise_exception(DaffyLib::KeyManagementService::KeyCreateException)
    end
    ...
    it 'raises an exception when the call to create an EncryptionKey fails (failure on RecordInvalid that is not a uniqueness violation)' do
      allow(DaffyLib::EncryptionKey).to receive(:create!).and_raise(ActiveRecord::RecordInvalid)
    ...
    it 'retries find on failure to create with uniqueness violation (RecordInvalid on a uniqueness violation)' do
      ...
      allow(DaffyLib::EncryptionKey).to receive(:find_by)
      allow(DaffyLib::EncryptionKey).to receive(:create!).and_raise(ActiveRecord::RecordInvalid, encryption_key)
    ...
    it 'retries find on failure to create with uniqueness violation (RecordNotUnique)' do
      allow(DaffyLib::EncryptionKey).to receive(:find_by)
      allow(DaffyLib::EncryptionKey).to receive(:create!).and_raise(ActiveRecord::RecordNotUnique)
    ...
    it 'returns an EncryptionKey on redis write failure' do
      ...
      expect(service.find_or_create_encryption_key(encryption_epoch)).to be_a(DaffyLib::EncryptionKey)
    end
    ...
    it 'creates a new key when one does not exist' do
      expect do
        service.find_or_create_encryption_key(encryption_epoch)
      end.to change(DaffyLib::EncryptionKey, :count).by(1)
    end
    ...
    it 'does not create a new key when one already exists' do
      ...
      expect do
        service.find_or_create_encryption_key(encryption_epoch)
      end.not_to change(DaffyLib::EncryptionKey, :count)
    end
    ...
    it 'returns the correct version' do
      expect(service.find_or_create_encryption_key(encryption_epoch).version).to eq(DaffyLib::KeyManagementService::KEY_VERSION)
    end
  end
  ...
  describe '#retrieve_plaintext_key' do
    ...
    it 'raises an exception when encryption_key is nil' do
      expect { service.retrieve_plaintext_key(nil) }.to raise_exception(DaffyLib::KeyManagementService::InvalidParameterException)
    end
    ...
    it 'raises an exception when the call to PorkyLib::Symmetric fails' do
      ...
      expect { service.retrieve_plaintext_key(encryption_key) }.to raise_exception(DaffyLib::KeyManagementService::KeyRetrieveException)
    end
